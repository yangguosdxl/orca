...
export function useCreatePullRequestDialogFields({
  ...
  useEffect(() => {
    ...
    if (fieldRevisionsRef.current.base !== 0) {
      return
    }
    // Why: the Source Control compare-base picker can change the intended
    // review target after the composer is already open; keep untouched fields aligned.
    setBase(resolvedDefaultBaseRef)
    setBaseQuery('')
    setBaseResults([])
    setBaseSearchError(null)
  }, [eligibility, open, resolvedDefaultBaseRef])
