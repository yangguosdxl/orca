...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      "Write-Output '{}'",
      '$orcaOriginalPort = $env:ORCA_AGENT_HOOK_PORT',
      '$orcaOriginalToken = $env:ORCA_AGENT_HOOK_TOKEN',
      '$orcaOriginalEnv = $env:ORCA_AGENT_HOOK_ENV',
      '$orcaOriginalVersion = $env:ORCA_AGENT_HOOK_VERSION',
      // Why: endpoint.cmd is cmd syntax, not PowerShell. Parse its `set KEY=...`
      // lines so surviving PTYs can refresh to the current Orca server. Keep
      // the PTY snapshot so a stale endpoint file cannot erase a valid launch.
      ...
      '  } catch {}',
      '}',
      'if ((-not $env:ORCA_AGENT_HOOK_PORT -and -not $orcaOriginalPort) -or (-not $env:ORCA_AGENT_HOOK_TOKEN -and -not $orcaOriginalToken) -or -not $env:ORCA_PANE_KEY) { exit 0 }',
      '$inputData = [Console]::In.ReadToEnd()',
      'if ([string]::IsNullOrWhiteSpace($inputData)) { exit 0 }',
      'try { $payload = $inputData | ConvertFrom-Json } catch { exit 0 }',
      'function Invoke-OrcaCopilotHook([string]$port, [string]$token, [string]$hookEnv, [string]$version) {',
      '  if ([string]::IsNullOrWhiteSpace($port) -or [string]::IsNullOrWhiteSpace($token)) { return $false }',
      '  try {',
      '    $body = @{',
      ...
      "    Invoke-WebRequest -UseBasicParsing -Method Post -Uri ('http://127.0.0.1:' + $port + '/hook/copilot') -Headers @{ 'Content-Type'='application/json'; 'X-Orca-Agent-Hook-Token'=$token } -Body $body -TimeoutSec 2 | Out-Null",
      '    return $true',
      '  } catch { return $false }',
      '}',
      'if (Invoke-OrcaCopilotHook $env:ORCA_AGENT_HOOK_PORT $env:ORCA_AGENT_HOOK_TOKEN $env:ORCA_AGENT_HOOK_ENV $env:ORCA_AGENT_HOOK_VERSION) { exit 0 }',
      'if ($orcaOriginalPort -and ($orcaOriginalPort -ne $env:ORCA_AGENT_HOOK_PORT -or $orcaOriginalToken -ne $env:ORCA_AGENT_HOOK_TOKEN)) {',
      '  [void](Invoke-OrcaCopilotHook $orcaOriginalPort $orcaOriginalToken $orcaOriginalEnv $orcaOriginalVersion)',
      '}',
      'exit 0',
      ''
    ].join('\r\n')
